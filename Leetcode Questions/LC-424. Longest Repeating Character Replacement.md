...
If the window is invalid, move the start pointer ahead by one step. Every time startstartstart moves forward, we update the frequency of the outgoing element in the map. At this point the size of the window is equal to the longest valid window we have seen so far. 
